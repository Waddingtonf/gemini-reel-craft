...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: processingVideos, error: selectError } = await supabaseClient.from('videos').select('*').eq('status', 'processing').not('operation_name', 'is', null).gte('created_at', twoHoursAgo);
    if (selectError) {
